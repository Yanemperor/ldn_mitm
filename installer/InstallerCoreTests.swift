...
struct InstallerCoreTests {
    static func main() throws {
        ...
        let overlay = card.appendingPathComponent("switch/.overlays/ldnmitm_config.ovl")
        try fm.createDirectory(at: overlay.deletingLastPathComponent(), withIntermediateDirectories: true)
        try Data("old-overlay".utf8).write(to: overlay)
        try Data("preserve".utf8).write(to: card.appendingPathComponent("config/ldn_mitm/unrelated.cfg"))
        let removed = try SwitchInstaller.removeHistoricalVersion(from: card)
        precondition(removed.count == 4)
        precondition(!fm.fileExists(atPath: card.appendingPathComponent("switch/RyuLink").path))
        precondition(!fm.fileExists(atPath: overlay.path))
        precondition(!fm.fileExists(atPath: card.appendingPathComponent("atmosphere/contents/4200000000000010").path))
        precondition(!fm.fileExists(atPath: card.appendingPathComponent("config/ldn_mitm/relay.cfg").path))
        precondition(fm.fileExists(atPath: card.appendingPathComponent("config/ldn_mitm/unrelated.cfg").path))
        precondition(fm.fileExists(atPath: card.appendingPathComponent("atmosphere/contents.disabled/420000000000000B/exefs.nsp").path))
        print("Installer core test passed: copied, hashed, backed up, and removed only RyuLink files.")
    }
}
