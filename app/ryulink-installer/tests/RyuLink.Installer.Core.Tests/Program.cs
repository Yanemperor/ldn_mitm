...
try
{
    ...
    if (result.Files.Count != 4 || !File.Exists(Path.Combine(result.BackupDirectory, "config", "ldn_mitm", "relay.cfg")) || !File.Exists(Path.Combine(result.BackupDirectory, "atmosphere", "contents.disabled", "420000000000000B", "exefs.nsp"))) throw new Exception("backup or required file validation failed");
    ...
    Console.WriteLine("Installer core test passed: backup, exact four-file copy, and SHA-256 verification.");
}
